...
mod parallel_prover_tests {
    ...
    fn benchmark_parallel_vs_sequential_proof_generation() {
        ...
        if std::env::var("CI").is_ok() || !std::env::args().any(|arg| arg == "--nocapture") {
            ...
            println!("Run with 'cargo test --test parallel_prover_benchmark -- --nocapture' to execute");
            return;
        }
        ...
        for &batch_size in &batch_sizes {
            ...
            let par2_proofs = generate_proofs_for_circuit(&circuit, witnesses.clone(), par2_options);
            let par2_time = par2_start.elapsed();
            println!("Parallel proof generation time (2 threads): {:?}", par2_time);
            ...
            let par4_proofs = generate_proofs_for_circuit(&circuit, witnesses.clone(), par4_options);
            let par4_time = par4_start.elapsed();
            println!("Parallel proof generation time (4 threads): {:?}", par4_time);
            ...
            let par_auto_proofs = generate_proofs_for_circuit(&circuit, witnesses, par_auto_options);
            let par_auto_time = par_auto_start.elapsed();
            println!("Parallel proof generation time (auto threads): {:?}", par_auto_time);
            ...
            let verify_results = verify_proofs_in_parallel(circuit_refs, proof_refs, ParallelProverOptions::default());
    ...
    fn benchmark_parallel_verification() {
        ...
        let par2_start = Instant::now();
        let par2_results = verify_proofs_in_parallel(circuit_refs.clone(), proof_refs.clone(), par2_options);
        let par2_time = par2_start.elapsed();
        ...
        let par4_start = Instant::now();
        let par4_results = verify_proofs_in_parallel(circuit_refs.clone(), proof_refs.clone(), par4_options);
        let par4_time = par4_start.elapsed();
        ...
        let par_auto_start = Instant::now();
        let par_auto_results = verify_proofs_in_parallel(circuit_refs, proof_refs, par_auto_options);
        let par_auto_time = par_auto_start.elapsed();
        println!("Parallel verification time (auto threads): {:?}", par_auto_time);
