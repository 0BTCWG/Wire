...
fn test_burn_proof_creation() {
    let burn_proof = BurnProof {
        ...
        txid: "burn-tx-1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef"
            .to_string(),
    ...
    assert_eq!(
        burn_proof.txid,
        "burn-tx-1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef"
    );
    assert_eq!(
        burn_proof.bitcoin_address,
    ...
    assert_eq!(
        burn_proof.status,
        wire_lib::mpc::burn::BurnProofStatus::Pending
    );
...
fn test_burn_proof_serialization() {
    ...
    // Deserialize
    let deserialized: BurnProof =
        serde_json::from_str(&serialized).expect("Failed to deserialize burn proof");
...
fn test_burn_manager() -> MPCResult<()> {
    ...
    let mut manager = BurnManager::new(mpc_core.clone(), db_path.to_str().unwrap().to_string())?;
    ...
    manager.update_burn_proof_status(
        &burn_proof.id,
