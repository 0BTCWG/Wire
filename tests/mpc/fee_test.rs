...
use wire_lib::mpc::fee::{
    FeeConsolidation, FeeConsolidationStatus, FeeManager, FeeUTXO, FeeUTXOStatus,
};
...
fn test_fee_utxo_creation() {
    ...
    assert_eq!(
        utxo.txid,
        "fee-tx-1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef"
    );
...
fn test_fee_consolidation_creation() {
    ...
    assert_eq!(
        consolidation.txid,
        Some("consolidation-tx-1234567890abcdef".to_string())
    );
}
...
fn test_fee_consolidation_serialization() {
    ...
    // Serialize
    let serialized =
        serde_json::to_string(&consolidation).expect("Failed to serialize consolidation");

    // Deserialize
    let deserialized: FeeConsolidation =
        serde_json::from_str(&serialized).expect("Failed to deserialize consolidation");
    ...
    assert_eq!(
        deserialized.destination_address,
        consolidation.destination_address
    );
...
fn test_fee_manager() -> MPCResult<()> {
    ...
    // Find the new UTXO
    let new_utxo = all_utxos
        .iter()
        .find(|u| {
            u.status == FeeUTXOStatus::Available && u.id != "test-utxo-10" && u.id != "test-utxo-11"
        })
        .unwrap();
