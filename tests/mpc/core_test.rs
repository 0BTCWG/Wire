...
fn test_mpc_config_serialization() {
    ...
    // Deserialize
    let deserialized: MPCConfig = serde_json::from_str(&serialized).expect("Failed to deserialize config");
...
fn test_signing_ceremony() -> MPCResult<()> {
    ...
    let mut ceremony = SigningCeremony::new("test-signing-1".to_string(), message.to_vec(), mpc_core.clone())?;
