...
use wire_lib::mpc::attestation::{MintAttestation, AttestationManager};
...
fn test_mint_attestation_creation() {
    let attestation = MintAttestation {
        ...
        bitcoin_txid: "1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef".to_string(),
        bitcoin_vout: 0,
    ...
    assert_eq!(attestation.bitcoin_txid, "1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef");
    ...
    assert_eq!(attestation.status, wire_lib::mpc::attestation::AttestationStatus::Pending);
...
fn test_mint_attestation_serialization() {
    ...
    // Deserialize
    let deserialized: MintAttestation = serde_json::from_str(&serialized).expect("Failed to deserialize attestation");
    ...
    assert_eq!(deserialized.recipient_pubkey_hash, attestation.recipient_pubkey_hash);
...
fn test_attestation_manager() -> MPCResult<()> {
    ...
    assert_eq!(retrieved.recipient_pubkey_hash, attestation.recipient_pubkey_hash);
    assert_eq!(retrieved.amount, attestation.amount);
    
    // Update the attestation status
    manager.update_attestation_status(&attestation.id, wire_lib::mpc::attestation::AttestationStatus::Signing)?;
    
    let updated = manager.get_attestation(&attestation.id)?;
    assert_eq!(updated.status, wire_lib::mpc::attestation::AttestationStatus::Signing);
    ...
    assert_eq!(signed.status, wire_lib::mpc::attestation::AttestationStatus::Signed);
...
fn test_attestation_workflow() -> MPCResult<()> {
    ...
    assert_eq!(signed_attestation.status, wire_lib::mpc::attestation::AttestationStatus::Signed);
...
fn test_invalid_attestation() -> MPCResult<()> {
    ...
    let result = manager.create_attestation(
        "invalid-txid".to_string(),
        0,
        [4u8; 32],
        10_000_000,
    );
