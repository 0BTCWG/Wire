...
fn test_amm_swap_flow() {
    ...
    assert!(verify_result.is_ok(), "Swap proof verification failed: {:?}", verify_result.err());
    ...
    assert!(expected_output_amount >= min_output_amount, 
...
fn test_amm_add_liquidity_flow() {
    ...
    assert!(verify_result.is_ok(), "Add liquidity proof verification failed: {:?}", verify_result.err());
    ...
    assert!(expected_lp_tokens >= min_lp_tokens, 
...
fn test_amm_remove_liquidity_flow() {
    ...
    assert!(verify_result.is_ok(), "Remove liquidity proof verification failed: {:?}", verify_result.err());
    ...
    assert!(expected_amount_a >= min_amount_a, 
        "Expected amount A {} is less than minimum {}", 
        expected_amount_a, min_amount_a);
    assert!(expected_amount_b >= min_amount_b, 
