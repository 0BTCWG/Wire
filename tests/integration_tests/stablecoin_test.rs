...
use wire::mpc::stablecoin::{PriceAttestation, RedeemAttestation, StablecoinMPC};
...
fn test_stablecoin_redeem_flow() {
    ...
    let redeem_attestation = mpc
        .process_redeem_request(redeem_request_path.to_str().unwrap())
        .unwrap();
    ...
    assert!(
        verify_result.is_ok(),
        "Redeem proof verification failed: {:?}",
        verify_result.err()
    );
...
fn test_stablecoin_cli_commands() {
    ...
    let status = Command::new("cargo")
        .args(["run", "--", "stablecoin", "list-redeem-requests"])
        .status()
        .expect("Failed to execute command");
