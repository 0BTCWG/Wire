...
fn test_lightning_mint_flow() {
    ...
    let attestation_result = mpc.generate_mint_attestation(&invoice_result.invoice_hash).unwrap();
...
fn test_lightning_burn_flow() {
    ...
    assert!(verify_result.is_ok(), "Burn proof verification failed: {:?}", verify_result.err());
    ...
    let process_result = mpc.process_burn_proof(
        &proof_result,
        ln_address,
        withdrawal_amount,
        fee,
