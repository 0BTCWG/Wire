...
use wire_lib::mpc::fee::{FeeUTXO, FeeManager};
use wire_lib::mpc::ceremonies::{DKGCeremony, SigningCeremony, CeremonyStatus};
...
fn test_full_lifecycle() -> MPCResult<()> {
    ...
    let attestation_db_path = temp_dir.path().join("attestations.json").to_str().unwrap().to_string();
    let burn_db_path = temp_dir.path().join("burns.json").to_str().unwrap().to_string();
    ...
    let config = MPCConfig {
        ...
        my_index: 0,
        key_share_path: temp_dir.path().join("key_share.json").to_str().unwrap().to_string(),
    ...
    let mut attestation_manager = AttestationManager::new(
        mpc_core.clone(),
        attestation_db_path,
    )?;
    ...
    attestation_manager.update_attestation_status(&attestation.id, wire_lib::mpc::attestation::AttestationStatus::Signing)?;
    ...
    burn_manager.update_burn_proof_status(&burn_proof.id, wire_lib::mpc::burn::BurnProofStatus::Processing)?;
    ...
    let fee_db_path = temp_dir.path().join("fees.json").to_str().unwrap().to_string();
    ...
    assert_eq!(completed_consolidation.status, FeeConsolidationStatus::Completed);
