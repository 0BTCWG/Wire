...
mod recursive_prover_tests {
    ...
    use wire_lib::utils::{aggregate_proofs, RecursiveProverOptions};
    ...
    fn benchmark_recursive_proof_aggregation() {
        ...
        if std::env::var("CI").is_ok() || !std::env::args().any(|arg| arg == "--nocapture") {
            ...
            println!(
                "Run with 'cargo test --test recursive_prover_benchmark -- --nocapture' to execute"
            );
            return;
        }
        ...
        for &batch_size in &batch_sizes {
            ...
            println!(
                "Generated {} individual proofs in {:?}",
                batch_size, proof_generation_time
            );
            ...
            for &step_size in &step_sizes {
                ...
                println!(
                    "Aggregation throughput: {:.2} proofs/second",
                ...
                println!(
                    "Verification throughput: {:.2} proofs/second",
    ...
    fn benchmark_recursive_vs_individual_verification() {
        ...
        println!(
            "Individual verification of {} proofs: {:?}",
            batch_size, individual_time
        );
        println!(
            "Individual verification throughput: {:.2} proofs/second",
        ...
        println!(
            "Aggregated verification of {} proofs: {:?}",
            batch_size, aggregated_time
        );
        println!(
            "Aggregated verification throughput: {:.2} proofs/second",
