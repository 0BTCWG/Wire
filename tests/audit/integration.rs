...
fn test_integration_cli_wrapped_mint() {
    ...
    let input_json = format!(
        ...
        recipient_pk_hash, amount, deposit_nonce, custodian_pk_x, custodian_pk_y,
    ...
    file.write_all(input_json.as_bytes()).expect("Failed to write input file");
    ...
    match status {
        Ok(exit_status) => {
            assert!(exit_status.success(), "CLI command failed with status: {}", exit_status);
            ...
            // Parse the proof
            let proof: SerializableProof = serde_json::from_str(&proof_json).expect("Failed to parse proof");
            
            // Verify the proof
            let verification_result = WrappedAssetMintCircuit::verify_proof(&proof);
            assert!(verification_result.is_ok(), "Failed to verify proof: {:?}", verification_result.err());
        },
        Err(err) => {
...
fn test_integration_cli_wrapped_burn() {
    ...
    let input_json = format!(
        ...
        owner_pubkey_hash, asset_id, amount, salt, sender_sk, sender_pk_x, sender_pk_y,
        signature_r_x, signature_r_y, signature_s, destination_btc_address
    );
...
fn test_integration_cli_transfer() {
    ...
    let input_json = format!(
        ...
        owner_pubkey_hash, asset_id, amount, salt, recipient_pk_hash, output_amount,
        sender_sk, sender_pk_x, sender_pk_y, signature_r_x, signature_r_y, signature_s,
        owner_pubkey_hash, asset_id, amount, salt, fee_amount, fee_reservoir_address_hash, nonce
    );
...
fn test_integration_wasm_wrapped_mint() {
    ...
    let result: serde_json::Value = result_js.into_serde().expect("Failed to convert result to Rust object");
...
fn test_integration_recursive_aggregation() {
    ...
    let (recipient_pk_hash1, amount1, deposit_nonce1, signature1) = utils::generate_test_attestation();
    ...
    let destination_btc_address2 = vec![0x76, 0xa9, 0x14, 0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef, 0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef, 0x88, 0xac];
    ...
    assert!(mint_result.is_ok(), "Failed to generate wrapped mint proof: {:?}", mint_result.err());
    ...
    // Verify that proof generation succeeds
    assert!(burn_result.is_ok(), "Failed to generate wrapped burn proof: {:?}", burn_result.err());
    ...
    assert!(aggregation_result.is_ok(), "Failed to aggregate proofs: {:?}", aggregation_result.err());
    ...
    assert!(verification_result.is_ok(), "Failed to verify aggregated proof: {:?}", verification_result.err());
}
...
fn test_integration_recursive_verification() {
    ...
    let mint_proof_json = serde_json::to_string(&mint_proof).expect("Failed to serialize mint proof");
    let burn_proof_json = serde_json::to_string(&burn_proof).expect("Failed to serialize burn proof");
    ...
    mint_file.write_all(mint_proof_json.as_bytes()).expect("Failed to write mint proof file");
    burn_file.write_all(burn_proof_json.as_bytes()).expect("Failed to write burn proof file");
    ...
    match aggregate_status {
        Ok(exit_status) => {
            assert!(exit_status.success(), "Aggregation command failed with status: {}", exit_status);
            ...
            match verify_status {
                Ok(exit_status) => {
                    assert!(exit_status.success(), "Verification command failed with status: {}", exit_status);
                },
                Err(err) => {
                    // If the command fails, it might be because the CLI is not available
                    // In that case, skip the test
