...
fn test_edge_case_max_field_value() {
    ...
    match result {
        Ok(proof) => {
            ...
            assert!(verification_result.is_ok(), "Failed to verify proof with max field value: {:?}", verification_result.err());
...
fn test_edge_case_zero_value() {
    ...
    match result {
        Ok(proof) => {
            ...
            assert!(verification_result.is_ok(), "Failed to verify proof with zero amount: {:?}", verification_result.err());
...
fn test_edge_case_max_inputs() {
    ...
    for _ in 0..max_inputs {
        input_utxos_data.push((owner_pubkey_hash.clone(), asset_id.clone(), input_amount, salt.clone()));
    }
    ...
    match result {
        Ok(proof) => {
            ...
            assert!(verification_result.is_ok(), "Failed to verify proof with maximum inputs: {:?}", verification_result.err());
...
fn test_edge_case_max_outputs() {
    ...
    match result {
        Ok(proof) => {
            // If proof generation succeeds, verify that the proof is valid
            let verification_result = TransferCircuit::verify_proof(&proof);
            assert!(verification_result.is_ok(), "Failed to verify proof with maximum outputs: {:?}", verification_result.err());
...
fn test_edge_case_empty_inputs() {
    ...
    assert!(result.is_err(), "Proof generation should fail for empty inputs");
