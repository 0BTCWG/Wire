...
fn main() {
    ...
    println!("  Circuit build time: {:?} (estimated)", recursive_result.circuit_build_time);
    println!("  Proof generation time: {:?} (estimated)", recursive_result.proof_time);
    println!("  Proof verification time: {:?} (estimated)", recursive_result.verify_time);

    // Print summary
    println!("\nBenchmark Summary");
    println!("================");
    print_benchmark_row("Operation", "Gates", "Proof Time", "Verify Time", "Throughput");
    print_benchmark_row("--------", "-----", "----------", "-----------", "----------");
...
fn print_benchmark_row(op: &str, gates: &str, proof: &str, verify: &str, throughput: &str) {
    println!("{:<20} {:<10} {:<15} {:<15} {:<10}", op, gates, proof, verify, throughput);
}
...
fn benchmark_transfer_circuit() -> BenchmarkResult {
    ...
    // Input UTXO fields
    let input_pubkey_hash = (0..32).map(|_| builder.add_virtual_target()).collect::<Vec<_>>();
    let input_asset_id = (0..32).map(|_| builder.add_virtual_target()).collect::<Vec<_>>();
    let input_amount = builder.add_virtual_target();
    let input_salt = builder.add_virtual_target();
    
    // Output UTXO fields
    let output_pubkey_hash = (0..32).map(|_| builder.add_virtual_target()).collect::<Vec<_>>();
    let output_asset_id = (0..32).map(|_| builder.add_virtual_target()).collect::<Vec<_>>();
