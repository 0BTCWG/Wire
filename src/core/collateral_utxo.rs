...
impl CollateralUTXOTarget {
    ...
    pub fn new(
        utxo: UTXOTarget,
        metadata: CollateralMetadataTarget,
    ) -> Self {
        Self {
            utxo,
            metadata,
    ...
    pub fn is_valid_for_redemption<F: RichField + Extendable<D>, const D: usize>(
        ...
        builder.connect(timelock_expired_bool_as_target, timelock_expired_eq_as_target);
        
        // 2. Verify that the issuance ID matches
        let correct_issuance = compare_vectors(
            builder,
            &self.metadata.issuance_id,
            issuance_id,
        );
        
        // 3. Verify that the collateral is sufficient
        // Calculate the current value of the collateral
        let collateral_value = builder.mul(
            self.utxo.amount_target,
            current_price,
        );
        
        // Check if the collateral value is >= the required amount
        let required_value = builder.mul(
            self.utxo.amount_target,
            self.metadata.lock_price,
        );
        ...
        // We can't directly assert that two BoolTargets are equal, so we'll convert them to Targets
        let sufficient_collateral_bool_as_target = builder.select(sufficient_collateral_bool, one, zero);
        let sufficient_collateral_eq_as_target = builder.select(sufficient_collateral_eq, one, zero);
        builder.connect(sufficient_collateral_bool_as_target, sufficient_collateral_eq_as_target);
...
impl CollateralRegistry {
    ...
    pub fn find_collateral_utxo<F: RichField + Extendable<D>, const D: usize>(
        ...
        if !self.collateral_utxos.is_empty() {
            ...
            let _matches = compare_vectors(
                builder,
                &utxo.metadata.issuance_id,
                issuance_id,
            );
