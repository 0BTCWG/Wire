...
use wire_lib::mpc::stablecoin::{StablecoinMPC, PriceAttestation, RedeemRequest, RedeemAttestation};
...
pub fn generate_price_attestation(output_file: &str) -> Result<(), String> {
    ...
    // Generate a new price attestation
    let attestation = mpc.generate_price_attestation()
    ...
    println!("  Timestamp: {}", StablecoinMPC::format_timestamp(attestation.timestamp));
    println!("  BTC/USD Price: {}", StablecoinMPC::format_price(attestation.btc_usd_price));
...
pub fn get_latest_price_attestation(output_file: &str) -> Result<(), String> {
    ...
    // Get the latest price attestation
    let attestation = mpc.get_latest_price_attestation()
...
pub fn mint_zusd(
    ...
    let input_utxo_commitment = input_utxo_data["commitment"].as_str()
        .ok_or_else(|| "Missing commitment in input UTXO".to_string())?;
    
    let input_utxo_nullifier = input_utxo_data["nullifier"].as_str()
        .ok_or_else(|| "Missing nullifier in input UTXO".to_string())?;
    
    let input_utxo_asset_id = input_utxo_data["asset_id"].as_u64()
        .ok_or_else(|| "Missing asset_id in input UTXO".to_string())?;
    
    let input_utxo_amount = input_utxo_data["amount"].as_u64()
        .ok_or_else(|| "Missing amount in input UTXO".to_string())?;
    
    let input_utxo_owner = input_utxo_data["owner"].as_str()
    ...
    let (_mpc_signature_r_x, _mpc_signature_r_y, _mpc_signature_s) = price_attestation_data.signature;
    ...
    println!("  BTC/USD Price: {}", StablecoinMPC::format_price(btc_usd_price));
...
pub fn submit_redeem_request(
    ...
    // Parse the user PKH
    let user_pkh_bytes = hex::decode(user_pkh)
        .map_err(|e| format!("Failed to decode user PKH: {}", e))?;
    
    // Submit the redeem request
    let request = mpc.submit_redeem_request(&user_pkh_bytes, zusd_amount)
    ...
    println!("  zUSD Amount: {} zUSD", zusd_amount / 1_000_000);
    println!("  Timestamp: {}", StablecoinMPC::format_timestamp(request.timestamp));
    println!("  Saved to: {}", output_file);
...
pub fn list_redeem_requests() -> Result<(), String> {
    ...
    // List pending redeem requests
    let requests = mpc.list_pending_redeem_requests()
...
pub fn process_redeem_request(
    ...
    let attestation = mpc.process_redeem_request(request_file)
    ...
    println!("  User PKH: {}", user_pkh_hex);
    println!("  zUSD Amount: {} zUSD", attestation.zusd_amount / 1_000_000);
    println!("  Timestamp: {}", StablecoinMPC::format_timestamp(attestation.timestamp));
    println!("  Saved to: {}", output_file);
...
pub fn redeem_zusd(
    ...
    let btc_usd_price = price_attestation_data.btc_usd_price;
    let (_price_signature_r_x, _price_signature_r_y, _price_signature_s) = price_attestation_data.signature;
    ...
    let (_redeem_signature_r_x, _redeem_signature_r_y, _redeem_signature_s) = redeem_attestation_data.signature;
