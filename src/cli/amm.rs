...
use wire_lib::circuits::{SwapCircuit, AddLiquidityCircuit, RemoveLiquidityCircuit, PoolStateTarget};
...
pub fn create_pool(
    ...
    info!("Creating new liquidity pool for tokens {} and {}", token_a_asset_id, token_b_asset_id);
...
pub fn swap_tokens(
    ...
    info!("Swapping tokens in pool {}", pool_state["pool_id"].as_str().unwrap_or("unknown"));
    ...
    let asset_id = u64::from_str_radix(asset_id_str, 16)
    ...
    if asset_id != token_a_asset_id && asset_id != token_b_asset_id {
        return Err(format!("Input token asset ID {} does not match either pool token", asset_id_str));
    }
    
    // Ensure the output token is the other pool token
    if output_asset_id != token_a_asset_id && output_asset_id != token_b_asset_id {
        return Err(format!("Output token asset ID {} does not match either pool token", output_asset_id));
    }
    ...
    if output_amount < min_output_amount {
        return Err(format!("Output amount {} is less than minimum requested {}", output_amount, min_output_amount));
    }
...
pub fn add_liquidity(
    ...
    info!("Adding liquidity to pool {}", pool_state["pool_id"].as_str().unwrap_or("unknown"));
    ...
    if lp_tokens < min_lp_tokens {
        return Err(format!("LP tokens minted {} is less than minimum requested {}", lp_tokens, min_lp_tokens));
    }
...
pub fn remove_liquidity(
    ...
    // Validate the input and output paths
    validate_file_path(lp_share_path, true)
        .map_err(|e| format!("Invalid LP share path: {}", e))?;
    validate_file_path(pool_state_path, true)
        .map_err(|e| format!("Invalid pool state path: {}", e))?;
    validate_output_file_path(output_path)
        .map_err(|e| format!("Invalid output path: {}", e))?;
    ...
    info!("Removing liquidity from pool {}", pool_state["pool_id"].as_str().unwrap_or("unknown"));
    ...
    if lp_share_pool_id != pool_id {
        return Err(format!("LP share pool ID {} does not match pool ID {}", lp_share_pool_id, pool_id));
    }
    ...
    if amount_a < min_amount_a {
        return Err(format!("Token A amount {} is less than minimum requested {}", amount_a, min_amount_a));
    }
    if amount_b < min_amount_b {
        return Err(format!("Token B amount {} is less than minimum requested {}", amount_b, min_amount_b));
    }
