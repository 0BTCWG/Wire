...
use wire_lib::circuits::{LNMintCircuit, LNBurnCircuit};
...
pub fn generate_ln_invoice(amount: u64, output_path: &str) -> Result<(), String> {
    ...
    info!("Generating Lightning Network invoice for {} satoshis", amount);
...
pub fn check_ln_payment_status(payment_hash: &str) -> Result<(), String> {
    ...
    info!("Checking status of Lightning Network payment: {}", payment_hash);
...
pub fn generate_ln_mint_proof(
    ...
    info!("Generating mint proof for Lightning Network payment: {}", attestation.payment_hash);
...
pub fn generate_ln_burn_proof(
    ...
    // Validate the input and output paths
    validate_file_path(utxo_path, true)
        .map_err(|e| format!("Invalid UTXO path: {}", e))?;
    validate_file_path(invoice_path, true)
        .map_err(|e| format!("Invalid invoice path: {}", e))?;
    validate_output_file_path(output_path)
        .map_err(|e| format!("Invalid output path: {}", e))?;
    
    // Read the UTXO file
    let utxo_data = fs::read_to_string(utxo_path)
        .map_err(|e| format!("Failed to read UTXO file: {}", e))?;
    
    // Parse the UTXO
    let utxo: Value = serde_json::from_str(&utxo_data)
        .map_err(|e| format!("Failed to parse UTXO: {}", e))?;
    ...
    // Convert payment hash to bytes
    let payment_hash_bytes = hex::decode(&invoice.payment_hash)
        .map_err(|e| format!("Invalid payment hash: {}", e))?;
    
    // Convert destination to bytes
    let destination_bytes = hex::decode(&invoice.recipient_pkh)
...
pub fn submit_ln_burn_proof(
    proof_path: &str,
    invoice_path: &str,
) -> Result<(), String> {
    // Validate the input paths
    validate_file_path(proof_path, true)
        .map_err(|e| format!("Invalid proof path: {}", e))?;
    validate_file_path(invoice_path, true)
        .map_err(|e| format!("Invalid invoice path: {}", e))?;
