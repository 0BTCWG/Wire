...
use crate::core::proof::{SerializableProof, deserialize_proof};
...
impl LNBurnCircuit {
    ...
    pub fn create_circuit() -> CircuitData<GoldilocksField, PoseidonGoldilocksConfig, 2> {
        ...
        let payment_hash: Vec<Target> = (0..HASH_SIZE).map(|_| builder.add_virtual_target()).collect();
        let amount = builder.add_virtual_target();
        let expiry = builder.add_virtual_target();
        let destination = (0..HASH_SIZE).map(|_| builder.add_virtual_target()).collect();
    ...
    pub fn generate_proof(
        ...
        // Set payment hash
        let payment_hash_target: Vec<Target> = (0..HASH_SIZE).map(|_| builder.add_virtual_target()).collect::<Vec<_>>();
        for i in 0..HASH_SIZE {
        ...
        let _payment_preimage_target: Vec<Target> = (0..HASH_SIZE).map(|_| builder.add_virtual_target()).collect::<Vec<_>>();
        for i in 0..HASH_SIZE {
            if i < payment_preimage.len() {
                pw.set_target(_payment_preimage_target[i], GoldilocksField::from_canonical_u64(payment_preimage[i] as u64));
            } else {
        ...
        let _recipient_pk_hash: Vec<Target> = (0..HASH_SIZE).map(|_| builder.add_virtual_target()).collect();
        for i in 0..HASH_SIZE {
            if i < recipient_pk_hash.len() {
                pw.set_target(_recipient_pk_hash[i], GoldilocksField::from_canonical_u64(recipient_pk_hash[i] as u64));
            } else {
        ...
        pw.set_target(_mpc_pk.point.x, GoldilocksField::from_canonical_u64(mpc_pk_x));
        pw.set_target(_mpc_pk.point.y, GoldilocksField::from_canonical_u64(mpc_pk_y));
...
mod tests {
    ...
    fn test_ln_burn_circuit_constraints() {
        ...
        // Create targets for the invoice
        let payment_hash: Vec<Target> = (0..HASH_SIZE).map(|_| builder.add_virtual_target()).collect();
        let _payment_preimage: Vec<Target> = (0..HASH_SIZE).map(|_| builder.add_virtual_target()).collect();
        let amount = builder.add_virtual_target();
    ...
    fn test_ln_burn_payment_hash_preimage_relationship() {
        ...
        let payment_hash: Vec<Target> = (0..HASH_SIZE).map(|_| builder.add_virtual_target()).collect::<Vec<_>>();
        let payment_preimage: Vec<Target> = (0..HASH_SIZE).map(|_| builder.add_virtual_target()).collect::<Vec<_>>();
        
        // Set up mismatched hash and preimage values
        for i in 0..HASH_SIZE {
            let hash_constant = builder.constant(GoldilocksField::from_canonical_u64(i as u64 + 1));
            let preimage_constant = builder.constant(GoldilocksField::from_canonical_u64(i as u64 + 2));
