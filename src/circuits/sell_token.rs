...
use crate::utils::constants::{DOMAIN_SELL_TOKEN, SCALING_FACTOR};
use crate::utils::nullifier::{
    compute_utxo_commitment_hash, compute_utxo_nullifier_target, UTXOTarget as NullifierUTXOTarget,
};

/// Circuit for selling tokens to a virtual CPMM pool
#[derive(Debug, Clone)]
pub struct SellTokenCircuit<F: RichField + Extendable<D>, const D: usize> {
...
impl<F: RichField + Extendable<D>, const D: usize> SellTokenCircuit<F, D> {
    ...
    pub fn prove<C: GenericConfig<D, F = F> + 'static>(
        ...
        // Verify that the user has enough tokens to sell
        let has_enough_tokens = builder.gte(self.input_utxo.amount, self.token_in_amount);
        builder.assert_one(has_enough_tokens.target);
        ...
        let (token_out_amount, new_virtual_state) = builder.if_else(
            is_token_a,
            // Token A input, Token B output
            {
                // Calculate new token A reserve
                let new_token_a_reserve = builder.add(
                    self.current_virtual_state.token_a_reserve,
                    self.token_in_amount,
                );
                ...
                let token_out_amount = builder.sub(
                    self.current_virtual_state.token_b_reserve,
                    new_token_b_reserve,
                );
                ...
                let meets_min_out = builder.gte(token_out_amount, self.min_token_out_amount);
                builder.assert_one(meets_min_out.target);
                ...
                (token_out_amount, new_virtual_state)
            },
            // Token B input, Token A output
            {
                // Calculate new token B reserve
                let new_token_b_reserve = builder.add(
                    self.current_virtual_state.token_b_reserve,
                    self.token_in_amount,
                );
...
mod tests {
    ...
    fn test_sell_token_circuit() {
        ...
        // Sell 10 token A
        let token_in_amount = builder.constant(F::from_canonical_u64(10 * SCALING_FACTOR));
        let min_token_out_amount = builder.constant(F::from_canonical_u64(19 * SCALING_FACTOR)); // Expect ~19.8 token B
