...
impl LNMintCircuit {
    ...
    pub fn build<F: RichField + Extendable<D>, const D: usize>(
        ...
        verify_message_signature(builder, &message, &self.attestation.signature, &self.mpc_pk);
    ...
    pub fn create_circuit() -> CircuitData<GoldilocksField, PoseidonGoldilocksConfig, 2> {
        ...
        let payment_hash = (0..HASH_SIZE)
            .map(|_| builder.add_virtual_target())
            .collect();
        let amount = builder.add_virtual_target();
        let recipient_pk_hash = (0..HASH_SIZE)
            .map(|_| builder.add_virtual_target())
            .collect();
    ...
    pub fn generate_proof(
        ...
        let payment_hash_target = (0..HASH_SIZE)
            .map(|_| builder.add_virtual_target())
            .collect::<Vec<_>>();
        for i in 0..HASH_SIZE {
...
mod tests {
    ...
    fn test_ln_mint() {
        ...
        assert!(
            ...
            "Mock proof verification should succeed"
        );
        assert!(
            verification_result.unwrap(),
            "Mock proof should verify as true"
        );
    }
