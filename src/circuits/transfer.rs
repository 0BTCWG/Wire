...
impl TransferCircuit {
    ...
    pub fn build<F: RichField + Extendable<D>, const D: usize>(
        ...
        for input_utxo in &self.input_utxos {
            // Calculate the nullifier using the domain-separated hash function
            let converted_utxo = convert_utxo_target(input_utxo);
            let nullifier = crate::utils::nullifier::compute_utxo_nullifier_target(
                builder,
                &converted_utxo,
            );
        ...
        let fee_nullifier = crate::utils::nullifier::compute_utxo_nullifier_target(
            builder,
            &converted_fee_utxo,
        );
        
        // Register the fee nullifier as a public input
        builder.register_public_input(fee_nullifier);
                ...
                owner_pubkey_hash_target: (0..HASH_SIZE)
                    .map(|j| {
                        if j < self.input_utxos[0].owner_pubkey_hash_target.len() {
                            let sender_pk_hash_bit = self.input_utxos[0].owner_pubkey_hash_target[j];
                            let zero = builder.zero();
    ...
    pub fn populate_witness(
        ...
        for (i, input_utxo_bytes) in input_utxos.iter().enumerate() {
            ...
            if input_utxo_bytes.len() < 4 * 32 {
                return Err(WireError::GenericError("Invalid input UTXO bytes length".to_string()));
            }
        ...
        for (i, recipient_pk_hash) in recipient_pk_hashes.iter().enumerate() {
            if i >= circuit.recipient_pk_hashes.len() {
                return Err(WireError::GenericError("Too many recipient public key hashes".to_string()));
            }
        ...
        for (i, &output_amount) in output_amounts.iter().enumerate() {
            if i >= circuit.output_amounts.len() {
                return Err(WireError::GenericError("Too many output amounts".to_string()));
            }
        ...
        if sender_pk.len() < 64 {
            return Err(WireError::GenericError("Invalid sender public key length".to_string()));
        }
        ...
        let sender_pk_x = GoldilocksField::from_canonical_u64(
            u64::from_le_bytes([
                sender_pk_x_bytes[0], sender_pk_x_bytes[1], sender_pk_x_bytes[2], sender_pk_x_bytes[3],
                sender_pk_x_bytes[4], sender_pk_x_bytes[5], sender_pk_x_bytes[6], sender_pk_x_bytes[7],
            ])
        );
        
        let sender_pk_y = GoldilocksField::from_canonical_u64(
            u64::from_le_bytes([
                sender_pk_y_bytes[0], sender_pk_y_bytes[1], sender_pk_y_bytes[2], sender_pk_y_bytes[3],
                sender_pk_y_bytes[4], sender_pk_y_bytes[5], sender_pk_y_bytes[6], sender_pk_y_bytes[7],
            ])
        );
        ...
        if sender_sig.len() < 96 {
            return Err(WireError::GenericError("Invalid sender signature length".to_string()));
        }
        ...
        // Convert bytes to field elements
        let sig_r_x = GoldilocksField::from_canonical_u64(
            u64::from_le_bytes([
                sig_r_x_bytes[0], sig_r_x_bytes[1], sig_r_x_bytes[2], sig_r_x_bytes[3],
                sig_r_x_bytes[4], sig_r_x_bytes[5], sig_r_x_bytes[6], sig_r_x_bytes[7],
            ])
        );
        
        let sig_r_y = GoldilocksField::from_canonical_u64(
            u64::from_le_bytes([
                sig_r_y_bytes[0], sig_r_y_bytes[1], sig_r_y_bytes[2], sig_r_y_bytes[3],
                sig_r_y_bytes[4], sig_r_y_bytes[5], sig_r_y_bytes[6], sig_r_y_bytes[7],
            ])
        );
        
        let sig_s = GoldilocksField::from_canonical_u64(
            u64::from_le_bytes([
                sig_s_bytes[0], sig_s_bytes[1], sig_s_bytes[2], sig_s_bytes[3],
                sig_s_bytes[4], sig_s_bytes[5], sig_s_bytes[6], sig_s_bytes[7],
            ])
        );
        ...
        if fee_input_utxo.len() < 4 * 32 {
            return Err(WireError::GenericError("Invalid fee input UTXO bytes length".to_string()));
        }
        ...
        let fee_amount_value = u64::from_le_bytes([
            fee_amount_bytes[0], fee_amount_bytes[1], fee_amount_bytes[2], fee_amount_bytes[3],
            fee_amount_bytes[4], fee_amount_bytes[5], fee_amount_bytes[6], fee_amount_bytes[7],
        ]);
        ...
        if fee_reservoir_address_hash.len() < HASH_SIZE {
            return Err(WireError::GenericError("Invalid fee reservoir address hash length".to_string()));
        }
