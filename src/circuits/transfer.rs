...
impl TransferCircuit {
    ...
    pub fn build<F: RichField + Extendable<D>, const D: usize>(
        ...
        let fee_nullifier = Self::calculate_nullifier(
            ...
            &self.fee_input_utxo.salt_target,
        );
        builder.register_public_inputs(&fee_nullifier);
        println!("DEBUG: Calculated and registered fee UTXO nullifier");

        // Register public inputs
        println!("DEBUG: Registering public inputs");

        // Register input UTXOs
        for (i, input_utxo) in self.input_utxos.iter().enumerate() {
            println!("DEBUG: Registering input UTXO {}", i);
            builder.register_public_inputs(&input_utxo.owner_pubkey_hash_target);
            builder.register_public_inputs(&input_utxo.asset_id_target);
            builder.register_public_input(input_utxo.amount_target);
            builder.register_public_inputs(&input_utxo.salt_target);
        }

        // Register output UTXOs
        for (i, output_utxo) in output_utxos.iter().enumerate() {
            println!("DEBUG: Registering output UTXO {}", i);
            builder.register_public_inputs(&output_utxo.owner_pubkey_hash_target);
            builder.register_public_inputs(&output_utxo.asset_id_target);
            builder.register_public_input(output_utxo.amount_target);
            builder.register_public_inputs(&output_utxo.salt_target);
        }

        // Register change UTXO
        println!("DEBUG: Registering change UTXO");
        builder.register_public_inputs(&change_utxo.owner_pubkey_hash_target);
        builder.register_public_inputs(&change_utxo.asset_id_target);
        builder.register_public_input(change_utxo.amount_target);
        builder.register_public_inputs(&change_utxo.salt_target);

        // Register fee UTXO
        println!("DEBUG: Registering fee UTXO");
        builder.register_public_inputs(&fee_utxo.owner_pubkey_hash_target);
        builder.register_public_inputs(&fee_utxo.asset_id_target);
        builder.register_public_input(fee_utxo.amount_target);
        builder.register_public_inputs(&fee_utxo.salt_target);

        // Verify signature
        println!("DEBUG: Adding signature verification");
        // Calculate sender public key hash
        let sender_pk_hash = Self::hash_point(builder, &self.sender_pk);
        
        // Verify that the sender's public key hash matches the input UTXO owner hash
        for i in 0..HASH_SIZE {
            builder.connect(
                sender_pk_hash[i],
                self.input_utxos[0].owner_pubkey_hash_target[i],
            );
        }
        
        // Create a message to sign (simplified for testing)
        // In a real implementation, we would hash all the relevant transaction data
        let mut message = Vec::new();
        message.extend_from_slice(&self.input_utxos[0].asset_id_target);
        message.push(self.input_utxos[0].amount_target);
        message.extend_from_slice(&output_utxos[0].owner_pubkey_hash_target);
        message.push(output_utxos[0].amount_target);
        
        // Verify the signature (simplified)
        // In a real implementation, we would use a proper EdDSA verification gadget
        let signature_valid = Self::verify_signature_simplified(
            builder,
            &self.sender_pk,
            &self.sender_sig,
            &message,
        ...
        println!("DEBUG: Added signature verification");
        println!("DEBUG: Build method completed");
        
        // Return the first output UTXO
        output_utxos[0].clone()
    }
    
    // Simplified signature verification function for testing
    fn verify_signature_simplified<F: RichField + Extendable<D>, const D: usize>(
        builder: &mut CircuitBuilder<F, D>,
        _public_key: &PublicKeyTarget,
        _signature: &SignatureTarget,
        _message: &[Target],
    ) -> Target {
