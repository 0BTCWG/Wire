...
impl NativeAssetBurnCircuit {
    ...
    pub fn build<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize>(
        ...
        let is_valid_amount =
            is_less_than_or_equal(builder, self.burn_amount, self.input_utxo.amount_target);
        builder.assert_one(is_valid_amount);

        // Calculate the change amount
        let change_amount = builder.sub(self.input_utxo.amount_target, self.burn_amount);
    ...
    pub fn generate_proof(
        ...
        circuit
            .build::<GoldilocksField, PoseidonGoldilocksConfig, 2>(&mut builder, owner_sk_target);
        ...
        let _ = pw.set_target(
            circuit.input_utxo.amount_target,
            GoldilocksField::from_canonical_u64(*input_amount),
        );
        ...
        let _ = pw.set_target(
            circuit.owner_pk.point.x,
            GoldilocksField::from_canonical_u64(owner_pk_x),
        );
        let _ = pw.set_target(
            circuit.owner_pk.point.y,
            GoldilocksField::from_canonical_u64(owner_pk_y),
        );

        // Set owner secret key
        let _ = pw.set_target(
            owner_sk_target,
            GoldilocksField::from_canonical_u64(owner_sk),
        );

        // Set burn amount
        let _ = pw.set_target(
            circuit.burn_amount,
            GoldilocksField::from_canonical_u64(burn_amount),
        );
    ...
    fn build_circuit() -> WireResult<(
        ...
        let _fee_reservoir_address_hash: Vec<Target> =
            (0..32).map(|_| builder.add_virtual_target()).collect();
        ...
        let proof_with_public_inputs =
            crate::core::proof::generate_proof(&circuit_data, PartialWitness::new())
                .map_err(|e| WireError::ProofError(ProofError::from(e)))?;
