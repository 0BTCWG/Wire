...
impl BitcoinSecurityManager {
    ...
    fn load_database(&mut self) -> MPCResult<()> {
        let data = fs::read(&self.db_path).map_err(|e| {
            MPCError::InternalError(format!("Failed to read security database: {}", e))
        })?;

        let db: SecurityDatabase = serde_json::from_slice(&data).map_err(|e| {
            MPCError::InternalError(format!("Failed to parse security database: {}", e))
        })?;
    ...
    fn save_database(&self) -> MPCResult<()> {
        ...
        let data = serde_json::to_vec(&db).map_err(|e| {
            MPCError::InternalError(format!("Failed to serialize security database: {}", e))
        })?;
        ...
        fs::write(&self.db_path, data).map_err(|e| {
            MPCError::InternalError(format!("Failed to write security database: {}", e))
        })?;
