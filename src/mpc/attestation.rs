...
use crate::mpc::ceremonies::{Ceremony, SigningCeremony};
...
impl AttestationManager {
    ...
    fn load_attestations(&mut self) -> MPCResult<()> {
        let data = fs::read(&self.db_path).map_err(|e| {
            MPCError::InternalError(format!("Failed to read attestations database: {}", e))
        })?;

        let attestations: Vec<MintAttestation> = serde_json::from_slice(&data).map_err(|e| {
            MPCError::InternalError(format!("Failed to parse attestations database: {}", e))
        })?;
    ...
    fn save_attestations(&self) -> MPCResult<()> {
        ...
        let data = serde_json::to_vec(&attestations).map_err(|e| {
            MPCError::InternalError(format!("Failed to serialize attestations: {}", e))
        })?;

        fs::write(&self.db_path, data).map_err(|e| {
            MPCError::InternalError(format!("Failed to write attestations database: {}", e))
        })?;
