...
use crate::mpc::ceremonies::{Ceremony, CeremonyStatus, DKGCeremony};
...
impl KeyRotationManager {
    ...
    pub fn new(mpc_core: MPCCore, db_path: String) -> MPCResult<Self> {
        ...
        if Path::new(&db_path).exists() {
            let data = fs::read(&db_path).map_err(|e| {
                MPCError::InternalError(format!("Failed to read key rotation database: {}", e))
            })?;

            let rotation: KeyRotationRecord = serde_json::from_slice(&data).map_err(|e| {
                MPCError::InternalError(format!("Failed to parse key rotation database: {}", e))
            })?;
    ...
    fn save_current_rotation(&self) -> MPCResult<()> {
        if let Some(rotation) = &self.current_rotation {
            let data = serde_json::to_vec(rotation).map_err(|e| {
                MPCError::InternalError(format!("Failed to serialize key rotation record: {}", e))
            })?;

            // Create parent directories if they don't exist
            if let Some(parent) = Path::new(&self.db_path).parent() {
                fs::create_dir_all(parent).map_err(|e| {
                    MPCError::InternalError(format!("Failed to create directories: {}", e))
                })?;
            }

            fs::write(&self.db_path, data).map_err(|e| {
                MPCError::InternalError(format!("Failed to write key rotation record: {}", e))
            })?;
        }
    ...
    pub fn initiate_rotation(
        ...
        if let Some(rotation) = &self.current_rotation {
            if rotation.status == KeyRotationStatus::Pending
        ...
        let ceremony = DKGCeremony::new(self.mpc_core.clone(), config.parties, config.threshold);
    ...
    pub fn start_rotation(&mut self, ceremony: &mut DKGCeremony) -> MPCResult<()> {
        ...
        let rotation = match &mut self.current_rotation {
            ...
            _ => {
                return Err(MPCError::InternalError(
                    "No pending key rotation".to_string(),
                ))
            }
        };
    ...
    pub fn complete_rotation(&mut self, ceremony: &DKGCeremony, _password: &str) -> MPCResult<()> {
        // Check if a rotation is in progress
        let rotation = match &mut self.current_rotation {
            Some(rotation) if rotation.status == KeyRotationStatus::InProgress => rotation,
            _ => {
                return Err(MPCError::InternalError(
                    "No key rotation in progress".to_string(),
                ))
            }
        };
        ...
        if ceremony.status() != CeremonyStatus::Completed {
            return Err(MPCError::CeremonyError(
                "DKG ceremony not completed".to_string(),
            ));
        }
    ...
    pub fn is_rotation_in_progress(&self) -> bool {
        match &self.current_rotation {
            Some(rotation) => {
                rotation.status == KeyRotationStatus::Pending
                    || rotation.status == KeyRotationStatus::InProgress
            }
            None => false,
        }
