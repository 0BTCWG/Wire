...
impl BurnManager {
    ...
    fn load_database(&mut self) -> MPCResult<()> {
        ...
        let db: BurnDatabase = serde_json::from_slice(&data)
            .map_err(|e| MPCError::InternalError(format!("Failed to parse burn database: {}", e)))?;
    ...
    fn save_database(&self) -> MPCResult<()> {
        ...
        let data = serde_json::to_vec(&db)
            .map_err(|e| MPCError::InternalError(format!("Failed to serialize burn database: {}", e)))?;
        
        fs::write(&self.db_path, data)
            .map_err(|e| MPCError::InternalError(format!("Failed to write burn database: {}", e)))?;
    ...
    pub fn process_burn_proof(&mut self, burn_proof: BurnProof) -> MPCResult<BitcoinWithdrawal> {
        ...
        if self.burn_proofs.contains_key(&burn_proof.txid) {
            return Err(MPCError::BitcoinError(format!(
                "Burn proof already processed: {}", burn_proof.txid
            )));
        }
        ...
        self.burn_proofs.insert(burn_proof.txid.clone(), burn_proof.clone());
        ...
        self.withdrawals.insert(withdrawal_id.clone(), withdrawal.clone());
    ...
    fn verify_burn_proof(&self, burn_proof: &BurnProof) -> MPCResult<()> {
        ...
        if burn_proof.amount == 0 {
            return Err(MPCError::BitcoinError("Burn amount cannot be zero".to_string()));
        }
    ...
    pub fn create_withdrawal_transaction(&mut self, withdrawal_id: &str) -> MPCResult<BitcoinTransaction> {
        let withdrawal = self.withdrawals.get(withdrawal_id)
            .ok_or_else(|| MPCError::BitcoinError(format!("Withdrawal not found: {}", withdrawal_id)))?;
