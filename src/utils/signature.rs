...
pub fn bytes_to_public_key(bytes: &[u8]) -> WireResult<VerifyingKey> {
    VerifyingKey::from_bytes(
        bytes
            .try_into()
            .map_err(|_| CryptoError::KeyError("Invalid public key bytes".to_string()))?,
    )
...
pub fn verify_signature_in_circuit_with_targets<F: RichField + Extendable<D>, const D: usize>(
    ...
    // 5. Compute R + h·A
    let h_a =
        optimized_scalar_multiply_targets(builder, (public_key.point.x, public_key.point.y), h);
    let r_plus_h_a = point_add_targets(builder, (signature.r_point.x, signature.r_point.y), h_a);
...
pub fn batch_verify_signatures<F: RichField + Extendable<D>, const D: usize>(
    ...
    assert_eq!(
        messages.len(),
        signatures.len(),
        "Number of messages must match number of signatures"
    );
    assert_eq!(
        messages.len(),
        public_keys.len(),
