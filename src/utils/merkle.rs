...
use crate::utils::hash::{
    poseidon_hash_two, poseidon_hash_two_targets, poseidon_hash_with_domain,
    poseidon_hash_with_domain_targets,
};
...
pub fn compute_merkle_root<F: RichField>(
    leaf: F,
    index: usize,
    siblings: &[F],
    height: usize,
...
pub fn verify_merkle_proof_targets<F: RichField + Extendable<D>, const D: usize>(
    ...
    let computed_root =
        compute_merkle_root_targets(builder, proof.leaf, &proof.index_bits, &proof.siblings);
...
pub fn generate_merkle_proof<F: RichField>(tree: &[Vec<F>], leaf_index: usize) -> MerkleProof<F> {
