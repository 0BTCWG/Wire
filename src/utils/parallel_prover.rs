...
pub fn generate_proofs_in_parallel<
    F: RichField + Extendable<D>,
...
>(
    ...
    if options.verbose {
        println!(
            "Generating {} proofs using {} threads",
            circuits.len(),
            num_threads
        );
    }
            ...
            s.spawn(move |_| {
                ...
                if verbose {
                    ...
                    *completed += 1;
                    println!(
                        "Generated proof {}/{} in {:?}",
                        *completed, total_count, generation_time
                    );
                }
            });
        }
...
pub fn generate_proofs_for_circuit<
    F: RichField + Extendable<D>,
    C: GenericConfig<D, F = F> + 'static,
    const D: usize,
>(
...
pub fn verify_proofs_in_parallel<
    F: RichField + Extendable<D>,
    C: GenericConfig<D, F = F> + 'static,
    const D: usize,
>(
    ...
    if options.verbose {
        println!(
            "Verifying {} proofs using {} threads",
            circuits.len(),
            num_threads
        );
    }
    ...
    let results = Arc::new(Mutex::new(
        Vec::<(usize, Result<(), anyhow::Error>)>::with_capacity(circuits.len()),
    ));
            ...
            s.spawn(move |_| {
                ...
                if verbose {
                    ...
                    *completed += 1;
                    println!(
                        "Verified proof {}/{} in {:?}",
                        *completed, circuit_count, verification_time
                    );
                }
            });
        }
    ...
    let verification_results: Vec<Result<(), anyhow::Error>> = results
        .iter()
        .map(|r| match &r.1 {
            Ok(()) => Ok(()),
            Err(e) => Err(anyhow::anyhow!("{:?}", e)),
