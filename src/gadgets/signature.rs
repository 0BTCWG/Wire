...
pub fn verify_message_signature<F: RichField + Extendable<D>, const D: usize>(
    ...
    if message.is_empty() {
        ...
        // 3.5 Compute R + h·A
        let h_a = crate::utils::signature::scalar_multiply_targets(builder, (pk_x, pk_y), h);
        let r_plus_h_a = crate::utils::signature::point_add_targets(builder, (r_x, r_y), h_a);
...
pub fn batch_verify_signatures<F: RichField + Extendable<D>, const D: usize>(
    ...
    if messages.len() == 1 {
        return verify_message_signature(builder, &messages[0], &signatures[0], &public_keys[0]);
    }
    ...
    let base_point = crate::utils::signature::get_base_point_targets(builder);
    let sum_weighted_s_b =
        crate::utils::signature::scalar_multiply_targets(builder, base_point, sum_weighted_s);
    ...
    let right_side =
        crate::utils::signature::point_add_targets(builder, sum_weighted_r, sum_weighted_h_a);
...
pub fn scalar_mul<F: RichField + Extendable<D>, const D: usize>(
    ...
    let result =
        crate::utils::signature::scalar_multiply_targets(builder, (point.x, point.y), scalar);
...
mod tests {
    ...
    #[test]
    fn test_invalid_signature() -> WireResult<()> {
        let config = CircuitConfig::standard_recursion_config();
        let mut builder = CircuitBuilder::<F, D>::new(config);
