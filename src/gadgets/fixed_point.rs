...
use crate::gadgets::arithmetic::{gt, gte, is_equal, lt, lte};
...
pub fn to_fixed_point<F: RichField + Extendable<D>, const D: usize>(
...
pub fn from_fixed_point<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    fixed_value: Target,
) -> Target {
    let scaling_factor = builder.constant(F::from_canonical_u64(FIXED_POINT_SCALING_FACTOR));
...
pub fn fixed_add<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    a: Target,
    b: Target,
) -> Target {
...
pub fn fixed_sub<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    a: Target,
    b: Target,
) -> Target {
...
pub fn fixed_mul<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    a: Target,
    b: Target,
) -> WireResult<Target> {
...
pub fn fixed_div<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    a: Target,
    b: Target,
) -> WireResult<Target> {
...
pub fn fixed_sqrt<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    a: Target,
) -> Target {
...
pub fn fixed_min<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    a: Target,
    b: Target,
) -> Target {
...
pub fn fixed_max<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    a: Target,
    b: Target,
) -> Target {
...
pub fn fixed_abs<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    a: Target,
) -> Target {
    let zero = builder.zero();
...
pub fn fixed_ceil<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    a: Target,
) -> Target {
...
pub fn fixed_round<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    a: Target,
) -> Target {
    // Get the integer part (scaled by FIXED_POINT_SCALING_FACTOR)
    ...
    let half_scaling_factor = builder.constant(F::from_canonical_u64(FIXED_POINT_SCALING_FACTOR / 2));
...
pub fn fixed_floor<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    a: Target,
) -> Target {
...
pub fn fixed_in_range<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    a: Target,
    min: Target,
    max: Target,
...
pub fn fixed_pow<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    a: Target,
    exponent: u64,
) -> WireResult<Target> {
...
mod tests {
    ...
    fn test_fixed_div() {
        ...
        // Create inputs
        let a = F::from_canonical_u64(15_000_000); // 15.0 in fixed point
        let b = F::from_canonical_u64(3_000_000); // 3.0 in fixed point
        ...
        let result = fixed_div(&mut builder, a_t, b_t).unwrap();
    ...
    fn test_fixed_sqrt() {
        ...
        let expected = F::from_canonical_u64(4_000_000); // 4.0 in fixed point
